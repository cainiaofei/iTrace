...
/**
 * Created by niejia on 15/4/17.
 */
public class UD_Call_Data_V_CSTI implements CSTI {
    public double bonus;
    ...
    public UD_Call_Data_V_CSTI(RelationInfo relationInfo, UseEdge useEdge) {
    ...
    public SimilarityMatrix improve(SimilarityMatrix matrix, TextDataset textDataset, SimilarityMatrix similarityMatrix) {
        ...
        while (originLinks.size() != 0) {
            ...
            if (oracle.isLinkAboveThreshold(source, target) && !improvedLinks.contains(source + "#" + target)) {
                ...
                ((CallDataRelationGraph) relationGraph).searhNeighbourVertexByCall(relationGraph.getCodeVertexByName(target), connectedVertexByCall);
                ((CallDataRelationGraph) relationGraph).searhNeighbourVertexByData(relationGraph.getCodeVertexByName(target), connectedVertexByData);
