 ...
 */
public class PruningCall_Connection implements CSTI {
    ...
    public PruningCall_Connection(RelationInfo prunedRelationInfo, RelationInfo fullRelationInfo, UseEdge useEdge, Double bonus_threshold) {
    ...
    public SimilarityMatrix improve(SimilarityMatrix matrix, TextDataset textDataset, SimilarityMatrix similarityMatrix) {
        ...
        for (String source : matrix.sourceArtifactsIds()) {
            ...
            for (String target : links.keySet()) {
                ...
                if (vertexDist.get(target) > 0) {
//                    if (!firstValidPiecesCode.contains(target)) {
                        double afterBonus = score * (1 + vertexBonus.get(target));
//                    System.out.println(" vertexBonus.get(target) = " + vertexBonus.get(target) );
//                    afterBonus = afterBonus < maxBonusedValue ? afterBonus : maxBonusedValue * 0.99;
...
//                            double v = bonus * (1.0 / (cur + 1));
//                                                       double v = bonus * (1.0 / (cur + 1) + extraBonus);
//                            double v = 1.0 / (cur + 1) * extraBonus;
                            double v = 1.0 / Math.pow(2, cur) * extraBonus;

//                            System.out.println("HaHa = " + 1.0 / Math.pow(2, cur));
                            vertexBonus.put(cv.getName(), v);
                        }
                    }
                }
            }
