 ...
 */
public class PruningCall_Data_Connection_ICSE_RankInitialRegionPointWithIR implements CSTI {
    ...
    public PruningCall_Data_Connection_ICSE_RankInitialRegionPointWithIR(RelationInfo prunedRelationInfo, RelationInfo fullRelationInfo, UseEdge useEdge, Double bonus_thresholdForCall, Double bonus_thresholdForData) {
...
//            System.out.println(" extendedByDataRelation = " + extendedByDataRelation.size() );
            ...
            Map<String, Double> firstCutPointsIRValueRank = new LinkedHashMap<>();

            for (String target : firstCut) {
                firstCutPointsIRValueRank.put(target, matrix.getScoreForLink(source, target));
            }
            Stack<String> firstCutPointsStack = new Stack<>();
            firstCutPointsIRValueRank = MapUtil.sortByValue(firstCutPointsIRValueRank);
            for (String s : firstCutPointsIRValueRank.keySet()) {
                System.out.println("map: " +source+"_"+ s + "_" + firstCutPointsIRValueRank.get(s));
                firstCutPointsStack.push(s);
            }

            Map<String, Double> firstCutPointsDegree = new LinkedHashMap<>();
            double degree = 0.9999;
            while (!firstCutPointsStack.isEmpty()) {
                firstCutPointsDegree.put(firstCutPointsStack.pop(), degree );
                degree = degree - 0.0001;
            }

            for (String s : firstCutPointsDegree.keySet()) {
                System.out.println("degree map: " +source+"_"+ s + "_" + firstCutPointsDegree.get(s));
            }
...
//            double maxBonusedValue = getMaxValueAfterBonus(matrix, source, allPiece);
            ...
            for (String target : links.keySet()) {
                ...
                if (firstValidPiecesCode.contains(target)) {
                    ...
                    linksList.add(new SingleLink(source, target, improvedScore * firstCutPointsDegree.get(target)));
