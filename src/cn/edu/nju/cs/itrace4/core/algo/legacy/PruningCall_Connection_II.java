 ...
 */
public class PruningCall_Connection_II implements CSTI {
    ...
    public PruningCall_Connection_II(RelationInfo prunedRelationInfo, RelationInfo fullRelationInfo, UseEdge useEdge, Double bonus_threshold) {
...
//            double maxBonusedValue = getMaxValueAfterBonus(matrix, source, firstValidPiecesCode);
            ...
            for (String target : links.keySet()) {
                if (!firstValidPiecesCode.contains(target)) {
                    ...
                    double sumBonus = 0.0;
                    for (String vertexInfirstCut : firstValidPiecesCode) {
                        double bonusValue = ((CallDataRelationGraphUnormalized) fullRelationGraph).computeCallBonus(target, vertexInfirstCut);
...
//                            System.out.println("HaHa = " + 1.0 / Math.pow(2, cur));
    ...
    public String getAlgorithmName() {
        return "PruningCall_Connection_II";
    }
