...
public class GetLineChartData {
	...
	public void getLineData() throws IOException, ClassNotFoundException {
		for (Project project : projects) {
			...
			for(String model:models) {
				...
				storeLineGraphData(result_ir,result_ud,result_cluster_two,result_cluster_four,fileName);
			}
		}
	...
	private void storeLineGraphData(Result result_ir, Result result_ud,
			Result result_cluster_two,Result result_cluster_four,
			String fileName) throws IOException {
		BufferedWriter bw = new BufferedWriter(new FileWriter(basePath+File.separator+fileName));
		String header = "percent;ir;ud;cluster(20%);cluster(40%)";
		List<Double> irList = result_ir.getPrecisionAtRecallByTen();
		List<Double> udList = result_ud.getPrecisionAtRecallByTen();
		List<Double> cluster_two_List = result_cluster_two.getPrecisionAtRecallByTen();
		List<Double> cluster_four_List = result_cluster_four.getPrecisionAtRecallByTen();
		bw.write(header);
		bw.newLine();
		for(int recall = 1,index=0; recall <=10;recall++,index++) {
			...
			sb.append(cluster_two_List.get(index)+";");
			sb.append(cluster_four_List.get(index)+";");
