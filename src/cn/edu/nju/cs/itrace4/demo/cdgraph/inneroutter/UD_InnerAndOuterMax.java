...
public class UD_InnerAndOuterMax implements CSTI{
	...
	public UD_InnerAndOuterMax(RelationInfo ri,double callThreshold,double dataThreshold,
				...
				if(oracle.isLinkAboveThreshold(req,represent) && index<=verifyCount){//if start
					...
					for(int vertexId:vertexList) {
						if(vertexId==localMaxId) {
							...
							outerBonusMap.put(vertexId, outerBonusWeight*localMaxScore);
						}
					}
...
//						"data:"+dataGraphs[nums[i]][nums[j]]);
	...
	public String getAlgorithmName() {
		return "UD_InnerAndOuterMax"+callThreshold+"_"+dataThreshold;
	}
